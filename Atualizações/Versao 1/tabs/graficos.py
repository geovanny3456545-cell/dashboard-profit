...
def render(df, df_raw):
    ...
    if chart_view == "Gráfico de Resultados":
        ...
        # Split and Shade logic
        for i in range(len(y_vals) - 1):
            x_pair = [x_seq[i], x_seq[i+1]]
            y_pair = [y_vals[i], y_vals[i+1]]
            
            if (y_pair[0] >= 0 and y_pair[1] >= 0):
                color = '#00fa9a'
                fillcolor = 'rgba(0, 250, 154, 0.2)'
            elif (y_pair[0] <= 0 and y_pair[1] <= 0):
                color = '#ff4d4d'
                fillcolor = 'rgba(255, 77, 77, 0.2)'
            else:
                x_cross = x_pair[0] - y_pair[0] * (x_pair[1] - x_pair[0]) / (y_pair[1] - y_pair[0])
                
                seg_color = '#00fa9a' if y_pair[0] >= 0 else '#ff4d4d'
                seg_fill = 'rgba(0, 250, 154, 0.2)' if y_pair[0] >= 0 else 'rgba(255, 77, 77, 0.2)'
                fig.add_trace(go.Scatter(
                    x=[x_pair[0], x_cross], y=[y_pair[0], 0],
                    mode='lines', line=dict(color=seg_color, width=2),
                    fill='tozeroy', fillcolor=seg_fill,
                    showlegend=False, hoverinfo='none'
                ))
                x_pair = [x_cross, x_pair[1]]
                y_pair = [0, y_pair[1]]
                color = '#00fa9a' if y_pair[1] >= 0 else '#ff4d4d'
                fillcolor = 'rgba(0, 250, 154, 0.2)' if y_pair[1] >= 0 else 'rgba(255, 77, 77, 0.2)'
        ...
        fig.add_trace(go.Scatter(
            ...
            name="Patrimônio",
            hovertemplate="Trade %{x}<br>Saldo: R$ %{y:,.2f}<extra></extra>"
        ))
        
        fig.update_layout(
            title="Série de Resultados (Equity Curve)",
            plot_bgcolor='#1e1e1e', paper_bgcolor='#1e1e1e', font=dict(color='#ddd'),
            xaxis_title="Sequência de Operações", yaxis_title="R$",
            showlegend=False
        )
        fig.add_hline(y=0, line_dash="dash", line_color="#444", line_width=1)
        st.plotly_chart(fig, use_container_width=True, key="chart_equity")
